...
public class Position extends UDF<INTEGER> {

	private static final long serialVersionUID = 1L;

	public Class valideParameterType(Class[] params) throws SemanticException {
		if (params.length != 2) {
			throw new SemanticException("Function(Position) should have two parameters");
		}
		if (!CHAR.class.isAssignableFrom(params[0]) && !CHAR.class.isAssignableFrom(params[1])) {

			throw new SemanticException("Function(Position) should have correnct types");
		}
	...
	public static INTEGER evaluate(Object[] args) {
		...
		CHAR chars = (CHAR) arg0;
		String s, m;

		if (chars.getAttribute().isCaseSensitive()) {
			s = ((CHAR) arg0).getString();
			m = ((CHAR) arg1).getString();

		} else {
			s = ((CHAR) arg0).getString().toLowerCase();
			m = ((CHAR) arg1).getString().toLowerCase();
		}
